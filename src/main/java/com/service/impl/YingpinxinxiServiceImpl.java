...
public class YingpinxinxiServiceImpl extends ServiceImpl<YingpinxinxiDao, YingpinxinxiEntity> implements YingpinxinxiService {
    ...
    @Override
	public PageUtils queryPage(Map<String, Object> params, Wrapper<YingpinxinxiEntity> wrapper, int role) {
		  Page<YingpinxinxiView> page =new Query<YingpinxinxiView>(params).getPage();
		List<YingpinxinxiView>list = baseMapper.selectListView(page,wrapper);
		if (!CollectionUtils.isEmpty(list)) {
			for (YingpinxinxiView view : list) {
				view.setRole(role);
			}
		}
